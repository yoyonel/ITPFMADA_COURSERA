...
//PulseOsc osc[nb_instruments];
ADSR env[nb_instruments];
SinOsc vib[nb_instruments];
...
{	
	...
	// set a vibrato
	Set_Vibrato( vib[i], 0 );
	// add a vibrato to oscillator
	Add_Vibrato_to_SinOsc( vib[i], osc[i] );
	// remove a vibrato to oscillator
	//Remove_Vibrato_to_SinOsc( vib[i], osc[i] );

	//Set_Delay( delay[i], 0 );
	//Add_FeedBack_Loop( delay[i] );
	//Add_Delay_to_SinOsc( delay[i], osc[i] );
