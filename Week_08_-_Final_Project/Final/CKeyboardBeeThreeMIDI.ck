...
public class CKeyboardBeeThreeMIDI extends CInstrumentMIDI
{
	...
	BeeThree keyboard_fingers[nb_fingers];
    ...
    fun void set_preset_0_CKBTM()
    {
        ...
        for(0 => int i; i<nb_fingers; i++)
        {
            ...
            keyboard_fingers[i] => keyboard_gains[i] => keyboard_envs[i];
        }
    }
    ...
    fun void set_note_from_midi_CKBTM( int _midi_note, int _finger )
    {
		...
		// set the finger
		freqs[_finger] => keyboard_fingers[_finger].freq;
		// activate the note
        Math.random2f( 0.5, 1.0 ) => keyboard_fingers[_finger].noteOn;
    }
    ...
    fun void update_CKBTM()
    {
		...
		// Normalize gain
		0 => int nb_playing_notes;
		for( 0 => int i; i < playing_a_note.cap(); i++) playing_a_note[i] +=> nb_playing_notes;
		if( nb_playing_notes ) update_gains( nb_playing_notes );
    }	

	//
	//
	fun void update_gains( int _nb_fingers_playing )
	{
		1.0 / _nb_fingers_playing => float normalize_gain;
		for(0 => int i; i<nb_fingers; i++)
        {
			//  normalisation gain
			normalize_gain => keyboard_gains[i].gain;
        }
	}
