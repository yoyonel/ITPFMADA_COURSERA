...
//
//init_instrument( global_gain_for_instruments_instrument, mando, TYPE_INSTRUMENT_MANDOLIN );
init_instrument( global_gain_for_instruments_instrument, sax, TYPE_INSTRUMENT_SAXOFONY );
//_______________________________________________________________________________
...
array_dates_instru_1[array_dates_instru_1.cap() - 1] + array_durations_instru_1[array_durations_instru_1.cap()-1] => int endPart;

1 => masterEnv.keyOn;

while( current_tick < endPart )
{
	//update_midi_instruments( current_tick, mando, TYPE_INSTRUMENT_MANDOLIN, 1 );
	update_midi_instruments( current_tick, sax, TYPE_INSTRUMENT_SAXOFONY, 2 );
...
*
*/
fun void update_midi_instruments( int _current_tick, UGen _instrument, int _type_instrument, int _offset_scale )
{
	...
	for( 0 => int id_instrument; id_instrument < nb_instruments; id_instrument++ )
	{
		...
		if( stateMidi == 0 )
		{
			...
			if( _type_instrument == TYPE_INSTRUMENT_MANDOLIN )
			{
				...
				//
				1 => mandolinSTK.noteOn; // pluck strings : velocity [0.0, 1.0]
				freq_note => mandolinSTK.freq;
			}
			else if( _type_instrument == TYPE_INSTRUMENT_SAXOFONY )
			{
				_instrument $ Saxofony @=> Saxofony saxofonySTK;
				//
				1 => saxofonySTK.noteOn;
				freq_note => saxofonySTK.freq;
			}
		}
		else if( stateMidi == 1 )
		{			
...
fun void init_instrument( float _scale_gain, UGen _instrument, int _type_instrument )
{
	...
	0 => int i;
	{
		...
		if( bUseEnvelope )	set_and_add_fx_envelope( env, 10::ms, 0::ms, 1.00, 50::ms, lastUGen ) @=> lastUGen;
