...
[ freqs_temp["A4"], freqs_temp["B4"], freqs_temp["C4"], freqs_temp["D4"], 
  freqs_temp["E4"], freqs_temp["F4#"], freqs_temp["G4#"] ] @=> freqs_scales["A4_minor_melodic_scale"];
...
fun float func_heart_06_x( float _t )
{
    ...
    // sin(t)
    Math.sin(_t) => float sin_t;
    // x = 16*sin(t')^3
    16.0*(sin_t*sin_t*sin_t) => float x;
...
fun float func_heart_06_y( float _t )
{
    ...
    13*Math.cos(_t) - 5*Math.cos(2*_t) - 2*Math.cos(3*_t) - Math.cos(4*_t) => float y;
...
SinOsc osc1_major_scale;	
// set the initial gain for this oscillator
0.2150 => osc1_major_scale.gain;
// Connect osc1 to network audio and
// add some FX: Chorus, Panoramic, Reverb to osc1
//NRev reverb => Chorus stk_chorus ;
//osc1_major_scale => Pan2 osc1_pan => dac;
osc1_major_scale => NRev reverb => Chorus stk_chorus => Pan2 osc1_pan => dac;	// add some FX: Chorus, Panoramic
// set the FX
0.121=>reverb.mix;
0.1=>stk_chorus.mix;
0.1275=>stk_chorus.modDepth;
5=>stk_chorus.modFreq;
...
TriOsc osc2_minor_scale;
// set the initial gain for this oscillator
osc1_major_scale.gain() / 6.0 => osc2_minor_scale.gain;
// Connect osc1 to network audio and
// add some FX: Delay, Panoramic to osc2
osc2_minor_scale => Delay stk_delay => Pan2 osc2_pan => dac;
...
128 => int osc_nb_subdivision_sound_sample;

// ------------------------------
// Effect: Panoramic
// ------------------------------
//0.07125 => float osc1_pan_max_amp;
0.5 => float osc1_pan_max_amp;
...
while(time_current_length_song <= time_max_length_song)
{
    ...
    func_heart_inc_indice * (0.05*Std.rand2f(0.25, 0.75)) => float delay_on_osc2;
    Math.sin(func_heart_indice + delay_on_osc2) * pi => func_heart_t2;  
    ...
    {        
        ...
        osc1_pan.pan() => float osc1_pan_start;
        Math.cos(func_heart_indice) * osc1_pan_max_amp => float osc1_new_pan;
        //Math.cos(now/1::second) * osc1_pan_max_amp => float osc1_new_pan;
        osc1_new_pan - osc1_pan_start => float osc1_pan_diff;
        Std.fabs( osc1_pan_diff/osc_nb_subdivision_sound_sample ) => float osc1_pan_step;
        0 => int indice;
        for( 0 => indice; indice <= osc_nb_subdivision_sound_sample; indice++)
        {
            ( indice / (osc_nb_subdivision_sound_sample $ float) ) => float osc1_pan_ratio;
            osc1_pan_start + osc1_pan_diff*osc1_pan_ratio => float osc1_new_pan;
            osc1_new_pan => osc1_pan.pan;
